...
import { Package, Plus, Edit, Trash2, Search, Shield, AlertCircle } from 'lucide-react';
    ...
    return (
        <PharmacistLayout>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div className="space-y-1">
                    <h1 className="text-4xl font-black text-gray-900 tracking-tight flex items-center">
                        <Package className="w-10 h-10 mr-4 text-amber-500" />
                        Inventory
                    </h1>
                    <p className="text-gray-500 font-medium">Monitor stock levels and manage medication details</p>
                </div>
                <button
                    onClick={handleAddNew}
                    className="bg-amber-500 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-amber-600 transition-all flex items-center shadow-xl shadow-amber-100 active:scale-95 group"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-5">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${lowStockCount > 0 ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'}`}>
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Rx Required</p>
                        <p className="text-2xl font-black text-gray-900">{products.filter(p => p.prescriptionRequired).length}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-md p-6 rounded-[2rem] shadow-sm mb-8 border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6">
                <form onSubmit={handleSearch} className="relative w-full max-w-xl group">
                    <input
                        ...
                        className="w-full pl-14 pr-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition-all font-medium text-gray-900 group-hover:bg-white"
                    />
                    <Search className="absolute left-5 top-1/2 -translate-y-1/2 h-6 w-6 text-gray-400 group-focus-within:text-amber-500 transition-colors" />
                </form>
            </div>
                                ...
                                <tr>
                                    <td colSpan="6" className="px-8 py-20 text-center">
                                        <div className="flex flex-col items-center gap-4">
                                            <div className="animate-spin rounded-full h-10 w-10 border-4 border-amber-500/20 border-b-amber-500"></div>
                                ...
                                products.map((product) => (
                                    <tr key={product._id} className="hover:bg-amber-50/30 transition-colors group">
                                        <td className="px-8 py-6">
                                            ...
                                            <div className="text-xs font-bold text-amber-600 mt-1 uppercase tracking-wider">{product.brand}</div>
                                        </td>
                                        <td className="px-8 py-6">
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <div className="flex items-center justify-end space-x-3 transition-opacity">
                                                <button 
                                                    ...
                                                    className="p-3 text-amber-600 hover:bg-amber-500 hover:text-white rounded-2xl transition-all shadow-sm hover:shadow-lg active:scale-90"
                                                    title="Edit Product"
                                                >
